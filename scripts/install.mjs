#!/usr/bin/env node
// One-line installer: npx github:kjmagnan1s/trim-hero
//
// Installs Trim Hero as a managed Claude Code plugin (marketplace add + install),
// falling back to a git clone into ~/.claude/skills/trim-hero when the CLI is
...
const REPO = 'kjmagnan1s/trim-hero';
const SKILLS_COPY = path.join(os.homedir(), '.claude', 'skills', 'trim-hero');
...
console.log('trim-hero installer\n');

if (fs.existsSync(SKILLS_COPY)) {
  console.log(`Already installed as a personal skill at ${SKILLS_COPY}.`);
  console.log('Update it with: git -C ~/.claude/skills/trim-hero pull');
...
if (!probe.missing) {
  ...
  if (addOk) {
    const install = run('claude', ['plugin', 'install', `trim-hero@trim-hero`]);
    if (install.ok || /already installed/i.test(install.out)) {
      console.log('Installed as a plugin: trim-hero@trim-hero');
      console.log('\nRestart Claude Code, then type /trim-hero (or ask "what\'s eating my context window").');
      process.exit(0);
    }
...
if (!clone.ok) {
  ...
  console.error(`\nManual install: git clone https://github.com/${REPO}.git ~/.claude/skills/trim-hero`);
  process.exit(1);
}
